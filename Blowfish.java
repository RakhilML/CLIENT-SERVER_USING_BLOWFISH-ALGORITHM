...
public class Blowfish {
    ...
    private long modVal = 1;

    private String hexToBin(String plainText) {
        StringBuilder binary = new StringBuilder();
        int n = plainText.length();
        for (int i = 0; i < n; i++) {
            long num = Long.parseUnsignedLong(String.valueOf(plainText.charAt(i)), 16);
            String binary4B = Long.toBinaryString(num);
    ...
    private String addBin(String a, String b) {
        ...
        n1 = (n1 + n2) % modVal;
        String ans = Long.toHexString(n1);
        ans = "00000000" + ans;
	...
	private String wordToHex(String word) {
	...
	private String hexToWord(String hexa) {
	...
	public String convertPlaintextToCiphertext(String plainText) {
		...
		String hexadecimal = wordToHex(plainText);
		...
		if (hexadecimal.length() == 16)
			...
			if (i < hexadecimal.length()) {
				String word = hexadecimal.substring(i);
		...
		list.forEach(hexValue -> System.out.println(hexValue));
		System.out.println();
	...
	public String convertCiphertextToPlaintext(String completeCiphertext) {
		...
		// Received Ciphertext will be split into words of length 16
		List<String> list = new ArrayList<>();
		int j = 0;
		...
		for (int i = 0; i < list.size(); i++) {
			...
			String newPlainText = hexToWord(newHexadecimal);
    ...
    public Blowfish(double key) {
        // Storing 2^32 in modVal
        for (int i = 0; i < 32; i++) {
            modVal = modVal << 1; // (<<1 is equivalent to multiply by 2)
        }

        System.out.println("Key in double format: " + Double.toString(key));
        ...
        String newKey = wordToHex(keyString);
        newKey = createLettersInKey(newKey);
        System.out.println("New key in hexadecimal format: " + newKey);
        keyGenerate(newKey);
    }
}
